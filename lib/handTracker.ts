...
// How long to wait on the wasm/model fetch before giving up and surfacing a
// clear "this is a network problem" error instead of hanging silently.
  ...
  async start(): Promise<void> {
    try {
      ...
      if (name === "NotAllowedError" || name === "PermissionDeniedError") {
        throw new TrackerError("camera_denied", "Camera permission was denied.", err);
      }
      if (name === "NotFoundError" || name === "OverconstrainedError") {
        throw new TrackerError("camera_not_found", "No camera was found on this device.", err);
      }
      if (name === "NotReadableError") {
        throw new TrackerError(
          "camera_in_use",
          "Camera is already in use by another application or tab.",
          err,
        );
      }
      throw new TrackerError("unknown", "Could not access the camera.", err);
    }
    ...
    try {
      ...
      throw new TrackerError("unknown", "Camera stream failed to start playing.", err);
    }
    ...
    try {
      fileset = await withTimeout(
        ...
        INIT_TIMEOUT_MS,
        "Timed out loading the MediaPipe runtime (wasm).",
      );
    } catch (err) {
      console.error("[HandTracker] wasm load failed:", err);
      throw new TrackerError(
        "wasm_load_failed",
        "Could not load the hand-tracking runtime. This is almost always a network/firewall issue " +
          `blocking ${WASM_CDN}.`,
        err,
      );
    }
    ...
    try {
      this.landmarker = await withTimeout(
        ...
        INIT_TIMEOUT_MS,
        "Timed out loading the hand-landmark model (GPU).",
      );
    } catch (gpuErr) {
      ...
      try {
        this.landmarker = await withTimeout(
          ...
          INIT_TIMEOUT_MS,
          "Timed out loading the hand-landmark model (CPU).",
        );
      } catch (cpuErr) {
        console.error("[HandTracker] CPU delegate also failed:", cpuErr);
        throw new TrackerError(
          "model_load_failed",
          `Could not load the hand-landmark model. This is almost always a network/firewall issue ` +
            `blocking ${MODEL_URL}, though it can also mean the browser lacks WebGL/WASM support.`,
          cpuErr,
        );
      }
    }
