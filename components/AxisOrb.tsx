...
import { HandTracker, TrackerError, type TrackerStatus } from "@/lib/handTracker";
  ...
  if (err instanceof TrackerError) {
    switch (err.code) {
      ...
      case "wasm_load_failed":
        return "TRACKING INIT FAILED — RUNTIME LOAD BLOCKED (CHECK NETWORK/FIREWALL, SEE CONSOLE)";
      case "model_load_failed":
        return "TRACKING INIT FAILED — MODEL LOAD BLOCKED (CHECK NETWORK/FIREWALL, SEE CONSOLE)";
      default:
        return "TRACKING INIT FAILED — SEE CONSOLE FOR DETAILS";
    }
  }
  return "TRACKING INIT FAILED — SEE CONSOLE FOR DETAILS";
}
...
export default function AxisOrb() {
  ...
  const startGestures = useCallback(async () => {
    ...
    try {
      ...
      console.error("[AxisOrb] gesture tracking failed to start:", err);
      trackerRef.current = null;
      tracker.stop();
      setCamera("error");
  ...
  return (
    <>
      ...
      <div className="hud hud-title">{aiThinking ? "A.X.I.S. — THINKING…" : "A.X.I.S."}</div>
      <div className="hud hud-subtitle">ANUJ SHUKLA // AI INTERFACE</div>
      ...
      <div className="hud hud-console">
        <AxisConsole onStateChange={(s) => setAiThinking(s === "thinking")} />
      </div>
